...
def download_map(route_number):
    base_url = "https://www.octranspo.com/images/files/routes_pdf/map_carte_"
    route_number_str = str(route_number).zfill(3)
    url = base_url + route_number_str + ".pdf"
...
def download_maps():
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(download_map, route_number) for route_number in range(1, 999)]
